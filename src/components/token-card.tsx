...
export function TokenCard({ id, name, ticker, image, creator, contributors, marketcap }: TokenCardProps) {
  return (
      ...
      <CardContent className="p-0">
        <div className="relative aspect-square w-full overflow-hidden rounded-t-md">
          ...
          <div className="absolute top-2 right-2 bg-jestr-background/80 backdrop-blur-sm rounded-full px-2 py-1">
            <span className="text-xs font-pixel text-jestr-yellow">${ticker}</span>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="flex items-center mt-1">
              ...
              {creator.platform === "x" ? (
                ...
                <span className="ml-1 text-xs text-muted-foreground">on Farcaster</span>
              )}
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex -space-x-2">
              {contributors.slice(0, 5).map((contributor, index) => (
                <div key={index} className="relative h-6 w-6 rounded-full border-2 border-jestr-card overflow-hidden">
                  <Image
              ...
              {contributors.length > 5 && (
                <div className="relative h-6 w-6 rounded-full bg-jestr-purple border-2 border-jestr-card flex items-center justify-center">
                  <span className="text-[10px] text-white">+{contributors.length - 5}</span>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <p className="font-pixel text-sm text-jestr-green">${marketcap.toLocaleString()}</p>
            </div>
